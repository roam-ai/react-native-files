...
@implementation RNRoam{
  BOOL hasListeners; // Flag to track if there are any listeners for events
}

- (dispatch_queue_t)methodQueue{
    // Ensures that React Native bridge calls are executed on the main thread
  return dispatch_get_main_queue();
}

RCT_EXTERN_METHOD(supportedEvents) // Declares a method to return supported event names

RCT_EXPORT_MODULE(); // Exposes this module to the React Native environment

- (instancetype)init {
  self = [super init];
  if (self) {
    // Sets the current class as the delegate for Roam SDK to receive location updates and other events
...
- (NSArray<NSString *> *)supportedEvents {
    // Returns an array of event names that the module can emit
